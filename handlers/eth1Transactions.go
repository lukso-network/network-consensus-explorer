		...
		for _, v := range t {
			wg.Go(func() error {
				...
				tableData = append(tableData, []interface{}{
					...
					toText,
					utils.FormatAmountFormatted(new(big.Int).SetBytes(v.GetValue()), "LYX", 8, 4, true, true, false),
					utils.FormatAmountFormatted(db.CalculateTxFeeFromTransaction(v, new(big.Int).SetBytes(b.GetBaseFee())), "LYX", 8, 4, true, true, false),
				})
				return nil
			})
			wg.Wait()
