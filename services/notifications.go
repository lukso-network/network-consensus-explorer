...
func (n *validatorBalanceDecreasedNotification) GetInfo(includeUrl bool) string {
	...
	generalPart := fmt.Sprintf(`The balance of validator %[1]v decreased for 3 consecutive epochs by %.9[2]f LYXt to %.9[3]f LYXt from epoch %[4]v to epoch %[5]v.`, n.ValidatorIndex, diff, balance, n.StartEpoch, n.EndEpoch)
	if includeUrl {
