...
func GetValidatorHist(validatorArr []uint64, currency string, start uint64, end uint64) rewardHistory {
	...
	for key, item := range totalIncomePerDay {
		...
		data[i] = []string{
			...
			addCommas(iETH, "%.5f"),                 // income of day LYXE
...
func GeneratePdfReport(hist rewardHistory) []byte {
	...
	// pdf.Ln(-1)
	pdf.CellFormat(0, maxHt, fmt.Sprintf("Total Income LYXE %s | %s", hist.TotalETH, hist.TotalCurrency), "", 0, "CM", true, 0, "")

	header := [colCount]string{"Date", "End-of-date balance LYXE", "Income for date LYXE", "Price of LYXE for date", "Income for date"}
	...
	header = [colCount]string{"Index", "Balance Activation LYXE", "Balance LYXE", "Income LYXE", "Last Attestation"}
