...
func FormatBalanceChange(balance *int64, currency string) template.HTML {
	...
	if currency == "LYXt" {
		...
		if balanceF < 0 {
			return template.HTML(fmt.Sprintf("<span title=\"%.0f GWei\" data-toggle=\"tooltip\" class=\"text-danger\">%.5f LYXt</span>", float64(*balance), balanceF))
		}
		return template.HTML(fmt.Sprintf("<span title=\"%.0f GWei\" data-toggle=\"tooltip\" class=\"text-success\">+%.5f LYXt</span>", float64(*balance), balanceF))
	} else {
		if balance == nil {
			return template.HTML("<span> 0 " + currency + "</span>")
		}
