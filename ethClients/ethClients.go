...
// Init starts a go routine to update the LYXE Clients Info
func Init() {
	go update()
}
...
func fetchClientData(repo string) *gitAPIResponse {
	...
	if err != nil {
		logger.Errorf("error retrieving LYXE Client Data: %v", err)
		return nil
	}
	...
	if err != nil {
		logger.Errorf("error decoding LYXE Clients json response to struct: %v", err)
		return nil
	}
...
func fetchClientNetworkShare() []ethernodesAPIStruct {
	...
	if err != nil {
		logger.Errorf("error retrieving LYXE Clients Network Share Data: %v", err)
	...
	if err != nil {
		logger.Errorf("error decoding LYXE Clients Network Share json response to struct: %v", err)
	}
...
func updateEthClient() {
	...
	logger.Println("Updating LYXE Clients Information")
	ethClientsMux.Lock()
